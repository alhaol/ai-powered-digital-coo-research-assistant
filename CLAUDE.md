...
The deck has no build step — open `presentation/index.html` in a browser. Arrows navigate; `S` speaker notes, `O` overview, `T` pacing timer, `P` export to PDF (`Shift+P` includes notes), `?` all keys. The current slide persists in the URL hash.
...
The three PDFs in `presentation/` are committed artifacts, generated by driving the deck's own `@media print` block (so the shipped file and a facilitator's `Ctrl+P` can't diverge). Regenerate them after any deck or booklet change — Playwright at `width=1280px height=720px` for the two decks, and `markdown` → A5 for the booklet.
...
**The QR code on slides 1 and 49 is baked in, not derived.** It's a hand-generated inline `<svg class="qr">` encoding the repo URL as a path — the picture will not follow the text if the URL changes, and a stale QR points people at the wrong repo while looking perfectly fine. Regenerate it (`pip install qrcode`), then **decode the render to prove it**, because a broken QR is invisible until someone scans it on the day:

```python
# qrcode.QRCode(error_correction=ERROR_CORRECT_M, border=2) -> get_matrix()
# merge each row's runs of dark modules into "M{x} {y}h{run}v1h-{run}z" subpaths
# then verify: cv2.QRCodeDetector().detectAndDecode(<screenshot of the rendered svg>)
```

**The pacing timer has two modes, and the difference is deliberate.** Idle, the clock *follows the deck* — `renderPace()` overwrites `acc` with `scheduledMin(n)`, so jumping to a slide shows where it falls in the 90 minutes (rehearsal). Running, it's real elapsed time and navigation must never touch it — a presenter's actual elapsed time is a fact, not a function of which slide they're on; the ahead/behind delta carries that information instead. Don't "simplify" these into one behaviour.
